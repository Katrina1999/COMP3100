    ...
    public void run() {
		...
		if (!globalString.equals("NONE")) {
			while (!end) {
				...
				Job job = new Job(
				    Integer.parseInt(jobString[1]),
				    Integer.parseInt(jobString[2]),
				    Integer.parseInt(jobString[3]),
				    Integer.parseInt(jobString[4]),
				    Integer.parseInt(jobString[5]),
			...
			//Add server information from string to serverArrList so algorithm performs on all the info
				...
				if (algorithmType.equals("bf")) {
				        ...
				        send("SCHD " + job.id + " " + sendTo.type + " " + sendTo.id);
				} else if (algorithmType.equals("sff")){
					sendTo = baselineAlgorithms.smartFF(job);
				        send("SCHD " + job.id + " " + sendTo.type + " " + sendTo.id);
				} else {
    ...
    public static void main(String args[])
    {
        ...
        if (args.length == 2) {
			if (args[0].equals("-a")) {
				...
				} else if (args[1].equals("sff")){
					client.algorithmType = "sff";
				}
			}
		}
        client.run();
